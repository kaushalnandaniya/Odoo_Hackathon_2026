...
import { requestPasswordReset, resetPassword, resendResetOTP } from "@/lib/actions/auth";
...
export function ForgotPasswordForm() {
  ...
  const [requestResult, requestAction, requestPending] = useActionState(requestPasswordReset, undefined);
  const [resetResult, resetAction, resetPending] = useActionState(resetPassword, undefined);
  ...
  useEffect(() => {
    if (requestResult?.startsWith("REQUIRE_OTP:")) {
      setRegisteredEmail(requestResult.replace("REQUIRE_OTP:", ""));
      setStep(2);
      setCooldown(60); 
    }
  }, [requestResult]);
  ...
  useEffect(() => {
    if (resetResult?.startsWith("SUCCESS:")) {
  ...
  const handleResend = async () => {
    ...
    const result = await resendResetOTP(formData);
      ...
      <AnimatePresence mode="wait">
        {step === 1 && (
          <motion.form 
            ...
            exit="exit"
          >
            {requestResult && !requestResult.startsWith("REQUIRE_OTP:") && (
              <motion.div 
        ...
        {step === 2 && (
          ...
          >
            <motion.div variants={itemVariants} className="bg-primary/5 border border-primary/20 p-4 rounded-2xl text-sm text-foreground/80 mb-6">
              We sent a 6-digit reset code to <span className="font-bold text-foreground">{registeredEmail}</span>.
            </motion.div>
            ...
            {resetResult && resetResult.startsWith("SUCCESS:") ? (
              <motion.div 
                initial={{ opacity: 0, y: -10, scale: 0.95 }} 
                animate={{ opacity: 1, y: 0, scale: 1 }} 
              ...
              >
                <div className="flex items-center gap-3 w-full">
                  ...
                  <span className="flex-1 text-left">{resetResult.replace("SUCCESS: ", "")}</span>
                </div>
            ...
            <motion.div variants={itemVariants} className="space-y-2 relative group">
              <label htmlFor="otp" className="block text-sm font-semibold text-foreground/80 ml-1">Reset Code</label>
              <div className="relative">
            ...
            <motion.div variants={itemVariants} className="space-y-2 relative group">
              <label htmlFor="password" className="block text-sm font-semibold text-foreground/80 ml-1">New Password</label>
              <div className="relative">
            ...
            <motion.button 
              variants={itemVariants}
              whileHover={resetResult?.startsWith("SUCCESS:") ? {} : { scale: 1.01, translateY: -2 }}
              whileTap={resetResult?.startsWith("SUCCESS:") ? {} : { scale: 0.98 }}
              type="submit" 
              disabled={resetPending || resetResult?.startsWith("SUCCESS:")}
              className="relative overflow-hidden w-full bg-primary hover:bg-primary/90 text-primary-foreground font-bold py-4 rounded-2xl transition-all disabled:opacity-70 mt-8 text-[1rem] tracking-wide shadow-lg shadow-primary/25 hover:shadow-xl hover:shadow-primary/30 flex items-center justify-center gap-2"
            >
              {resetPending ? (
                <>
              ...
              ) : resetResult?.startsWith("SUCCESS:") ? (
                "Success!"
              ) : (
                "Reset Password"
              )}
